; Sandbox Music — NSIS installer strings (English)
LangString welcomePageTitle ${LANG_ENGLISH} "Welcome to Sandbox Music Setup"
LangString welcomePageBody ${LANG_ENGLISH} "This installer sets up Sandbox Music with Sandbox Server — your local-first music console for locker storage, discovery, and playback.$\r$\n$\r$\nIncluded in this install:$\r$\n  • Sandbox Music desktop app$\r$\n  • Sandbox Server (tier34) — bundled Node.js runtime + server$\r$\n$\r$\nOn first launch you'll choose your locker folder. Sandbox Server starts automatically on this PC.$\r$\n$\r$\nInstalls to your user profile (no admin prompt). Re-running setup upgrades in place.$\r$\nClick Next to install."

LangString finishPageTitle ${LANG_ENGLISH} "Sandbox Music installed"
LangString finishPageBody ${LANG_ENGLISH} "Setup is complete. Sandbox Music and Sandbox Server are installed together on this PC.$\r$\n$\r$\nOpen Sandbox Music from the Start Menu — the server starts automatically. Point phones at this PC's LAN address (Settings → Vault on desktop shows the URL).$\r$\n$\r$\nYou can change locker folder and server options anytime under Settings → Vault."
...
LangString runApp ${LANG_ENGLISH} "&Run Sandbox Music"
